...
class PositionController(object):
    ...
    def __init__ (self):
		...
		self.Kp_psi = 1#0.008 
				#Kp =1 and Kv = 30 work nice but there is a small offset in the yaw direction
				#it does the first round pretty well and then diverges at the third quadrant
				#tuning KP below 1 would require Kv to go below 1 as well
				#increasing Kv beyond 30 causes drone to spin out - 
				#diversion happens suddenly after the third quadrant and then trajectory is normal again.IS there an issue after 270 degrees? 
				#Kv affects how it behaves on the third quadrant..inccreasing it further causes massive deviation
    ...
    def pos_cont(self,actual_pos, desired_pos, actual_vel, desired_vel):
		...
		z_acc = self.Kp_z*(zdes - zact) + self.Kv_z*(zdes_dot - z_dot)
		...
		psi_acc = self.Kp_psi * delta_psi + self.Kv_psi * (psi_des_dot - psi_dot)
		
		print("psi_acc = ", psi_acc, "delta_psi = ", delta_psi)
		...
		f = (z_acc + self.g)/(np.cos(theta_act)*np.cos(phi_act))
		...
		phi_c = phi_c*np.cos(psi_act) + theta_c*np.sin(psi_act)
		theta_c = -phi_c*np.sin(psi_act) + theta_c*np.cos(psi_act)

		return phi_c, theta_c, z_acc, psi_acc
